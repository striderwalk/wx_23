...
class gui(qt.QWidget):
    ...
    def MyUi(self):
        ...
        self.input_mqtt_topic.addItems(
            [
                "ac_phys/workxp/live_signals",
                "ac_phys/workxp/standard",
                "ac_phys/workxp/test_signals",
            ]
        )
        ...
        self.checkBox_65 = QtWidgets.QCheckBox(
            "SUM - R1  - R2", self.scrollAreaWidgetContents
        )
        ...
        self.button_clear_all = qt.QPushButton(
            "Clear Selected", self.scrollAreaWidgetContents
        )
    ...
    def update_plot(self, data):
        ...
        if self.checkBox_103.isChecked() == True:
            self.ax.plot(
                np.linspace(-0.5, 10.5, num=2200, endpoint=True),
                ((data[0])),
                c="blue",
                ls="-",
            )
        ...
        if self.checkBox_63.isChecked() == True:
            check_on[39] = 1
            self.ax.plot(
                np.linspace(-0.5, 10.5, num=2200, endpoint=True), ((data[39])), c="navy"
            )
        if self.checkBox_64.isChecked() == True:
            tot_sum = np.sum((data), axis=0)
            blm_sum = tot_sum - (data[0]) - (data[39])
            self.ax.plot(
                np.linspace(-0.5, 10.5, num=2200, endpoint=True), blm_sum, c="black"
            )
        if self.checkBox_65.isChecked() == True:
            ...
            self.ax.plot(
                np.linspace(-0.5, 10.5, num=2200, endpoint=True), blm_sum2, c="gray"
            )
        if 1 in check_on:
            pass
        else:
